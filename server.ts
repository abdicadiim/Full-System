...
const setSessionCookie = (res: express.Response, userId: string) => {
  ...
  const token = jwt.sign({ sub: userId } satisfies SessionClaims, JWT_SECRET, { expiresIn: "7d" });
...
const getAuthedUser = async (req: express.Request) => {
  if (AUTH_BYPASS) {
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  if (!MONGODB_URI) return res.status(500).json({ success: false, message: "DB not configured", data: null });
  if (!JWT_SECRET) return res.status(500).json({ success: false, message: "JWT_SECRET not configured", data: null });
