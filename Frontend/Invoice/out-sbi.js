...
const normalizeReportingTagAppliesTo = (tag) => {
  ...
  const fromModulesObject = tag?.modules && typeof tag.modules === "object" ? Object.keys(tag.modules).filter((key) => Boolean(tag.modules[key])) : [];
  const fallback = String(tag?.moduleLevel || tag?.module || "").split(",").map((entry) => entry.trim()).filter(Boolean);
  return [...direct, ...fromModulesObject, ...fallback].map((entry) => String(entry || "").trim().toLowerCase()).filter(Boolean);
};
...
const getMoreFilterFieldLabel = (field, reportingTagOptions = []) => {
...
const getMoreFilterValuePlaceholder = (field, locations = []) => {
...
const getMoreFilterValueLabel = (field, value, locations = []) => {
  if (!field) return "Select a value";
  if (field === "location") {
...
function SalesByItemReportView({
  categoryName,
  reportName,
  menuButtonRef,
  onMenuClick,
  ...
  const defaultDateRangeKey = "all-time";
  ...
  const [customDateRange, setCustomDateRange] = useState(() => getDateRangeValue(defaultDateRangeKey));
  const [customDateRangeDraft, setCustomDateRangeDraft] = useState(() => getDateRangeValue(defaultDateRangeKey));
  const [customDateRangeMonth, setCustomDateRangeMonth] = useState(() => getStartOfMonth(getDateRangeValue(defaultDateRangeKey).start));
  const [entityKeys, setEntityKeys] = useState(() => ENTITY_OPTIONS.map((option) => option.key));
  ...
  useEffect(() => {
    ...
    const loadFilterMetadata = async () => {
      try {
        ...
        const reportingTagRows = Array.isArray(reportingTagsResponse) ? reportingTagsResponse : Array.isArray(reportingTagsResponse?.data) ? reportingTagsResponse.data : [];
        const activeReportingTags = reportingTagRows.filter((tag) => tag?.isActive !== false).map((tag) => ({
        ...
        const itemReportingTags = activeReportingTags.filter(
          (tag) => (tag.appliesTo || []).some((entry) => entry.toLowerCase().includes("item"))
        );
        if (mounted) {
  ...
  const getCustomPickerSeedRange = (currentRange, currentKey) => currentKey === "all-time" && currentRange.start.getTime() === getDateRangeValue("all-time").start.getTime() ? getDateRangeValue("this-month") : currentRange;
  ...
  const reportColumnTotals = useMemo(
    () => visibleReportColumns.map((option) => {
      ...
      if (option.key === "average-price") {
        const totalQuantity = reportRows.reduce((sum, row) => sum + Number(row.values["quantity-sold"] || 0), 0);
        const totalAmount = reportRows.reduce((sum, row) => sum + Number(row.values.amount || 0), 0);
        return formatCurrency(totalQuantity !== 0 ? totalAmount / totalQuantity : 0, reportCurrency || "SOS");
      }
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      ...
      setReportError("");
      try {
        const response = await reportsAPI.getSalesByItem(buildSalesByItemQuery());
        if (cancelled) return;
  ...
  const getFilteredComparatorOptions = (query, field) => {
    const normalizedQuery = query.trim().toLowerCase();
    const fieldSpecificOptions = typeof field === "string" && field.startsWith("reporting-tag:") ? MORE_FILTER_COMPARATOR_OPTIONS.filter((option) => ["is-empty", "is-not-empty"].includes(option.key)) : field === "item-name" || field === "sku" || field === "usage-unit" ? MORE_FILTER_COMPARATOR_OPTIONS.filter(
      (option) => ["is-empty", "is-not-empty", "starts-with", "ends-with", "contains", "does-not-contain"].includes(option.key)
    ) : field === "location" ? MORE_FILTER_COMPARATOR_OPTIONS.filter((option) => ["is-in", "is-not-in"].includes(option.key)) : field === "customer-name" ? MORE_FILTER_COMPARATOR_OPTIONS.filter((option) => ["is-empty", "is-not-empty", "is-in", "is-not-in", "starts-with", "ends-with", "contains", "does-not-contain"].includes(option.key)) : MORE_FILTER_COMPARATOR_OPTIONS;
    return fieldSpecificOptions.filter((option) => option.label.toLowerCase().includes(normalizedQuery));
  };
  const getFilteredValueOptions = (field) => {
    if (!field) return [];
  ...
  const handleExportAction = (label) => {
    ...
    const fileBase = `sales-by-item-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}`;
        ...
        /* @__PURE__ */ jsxs(
          "button",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsx(ChevronDown, { size: 14, className: "pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#334155]" })
            ]
          }
        ),
      ...
      isMoreFiltersOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+10px)] z-40 w-[720px] rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
        /* @__PURE__ */ jsxs("div", { className: "px-4 py-4", children: [
          /* @__PURE__ */ jsx("div", { className: "space-y-3", children: moreFilterRows.map((row, index) => {
            ...
            const valueMode = typeof row.field === "string" && row.field.startsWith("reporting-tag:") ? "none" : row.comparator && MORE_FILTER_NO_VALUE_COMPARATORS.includes(row.comparator) ? "none" : row.comparator && MORE_FILTER_TEXT_COMPARATORS.includes(row.comparator) ? "text" : "dropdown";
            ...
            const valueLabel = valueMode === "text" ? row.value || "Enter a value" : row.value ? getMoreFilterValueLabel(row.field, row.value, availableLocations) : getMoreFilterValuePlaceholder(row.field, availableLocations);
            return /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-[34px_minmax(0,240px)_minmax(0,170px)_minmax(0,1fr)_auto_auto] items-center gap-3", children: [
                  ...
                  /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                    /* @__PURE__ */ jsx(
                      "input",
                      {
                        ...
                        onChange: (event) => setMoreFilterDropdown((prev) => prev ? { ...prev, search: event.target.value } : prev),
                        placeholder: "Search",
                        className: "h-9 w-full rounded-md border border-[#1b6f7b] bg-white pl-8 pr-3 text-sm text-[#334155] outline-none placeholder:text-[#94a3b8]"
                      }
                    ),
                ...
                /* @__PURE__ */ jsxs("div", { className: `flex min-h-[640px] flex-1 overflow-hidden ${customizeReportTab === "columns" ? "flex-col" : ""}`, children: [
                  ...
                  /* @__PURE__ */ jsx("main", { className: `min-w-0 flex-1 overflow-auto ${customizeReportTab === "columns" ? "px-5 py-6" : "px-6 py-6"}`, children: customizeReportTab === "general" ? /* @__PURE__ */ jsxs("div", { className: "space-y-6", children: [
                    ...
                    /* @__PURE__ */ jsxs("section", { ref: customizeMoreFiltersRef, children: [
                      ...
                      /* @__PURE__ */ jsx("p", { className: "mt-1 text-sm text-[#64748b]", children: "Use advanced filters to filter the report based on the fields of Items, Reports." }),
...
export {
  SalesByItemReportView as default
};
