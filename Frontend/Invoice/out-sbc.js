...
const formatPickerDate = (value) => value.toLocaleDateString("en-US", {
  month: "2-digit",
  day: "2-digit",
  year: "numeric"
});
...
const MORE_FILTER_TEXT_COMPARATORS = [
  "starts-with",
  "ends-with",
  "contains",
  "does-not-contain"
];
const MORE_FILTER_NO_VALUE_COMPARATORS = [
  "is-empty",
  "is-not-empty"
];
...
const getReportColumnOption = (key) => REPORT_COLUMN_GROUPS.flatMap((group) => group.options).find(
  (option) => option.key === key
);
const getReportColumnLabel = (key) => getReportColumnOption(key).label;
const getReportColumnGroupLabel = (key) => {
  const group = REPORT_COLUMN_GROUPS.find(
    (section) => section.options.some((option) => option.key === key)
  );
  return group?.label ?? "Reports";
};
const getReportColumnOptionForReport = (key, reportId) => {
  const option = getReportColumnOption(key);
  const labelOverride = REPORT_MODE_COLUMN_LABEL_OVERRIDES[reportId]?.[key] ?? option.label;
  return {
    ...option,
    label: labelOverride
  };
...
const COMPARE_WITH_NUMBER_OPTIONS = Array.from(
  { length: 35 },
  (_, index) => String(index + 1)
);
const REPORT_MODE_COLUMN_LABEL_OVERRIDES = {
  "sales-by-customer": {},
...
const formatBalanceCurrency = (value, currency = "SOS") => {
...
function ReportsDrawer({
  ...
  useEffect(() => {
    setExpandedSections(
      (prev) => prev.includes(currentCategoryId) ? prev : [currentCategoryId]
    );
  }, [currentCategoryId]);
  useEffect(() => {
    if (!open) return;
  ...
  const sections = useMemo(() => {
    ...
    return REPORTS_DRAWER_SECTIONS.map((section) => {
      ...
      const filteredReports = query ? reports.filter(
        (report) => report.name.toLowerCase().includes(query) || section.label.toLowerCase().includes(query)
      ) : reports;
              ...
              /* @__PURE__ */ jsxs(
                "button",
                {
                  ...
                  onClick: () => {
                    ...
                    setExpandedSections(
                      (prev) => prev.includes(section.id) ? [] : [section.id]
                    );
                  },
...
function SalesByCustomerReportView({
  ...
  const isCustomerBalanceSummaryReport = reportId === "customer-balance-summary";
  const isBadDebtsReport = reportId === "bad-debts";
  const isBankChargesReport = reportId === "bank-charges";
  const defaultDateRangeKey = isSalesByCustomerReport ? "all-time" : "this-month";
  const [dateRangeKey, setDateRangeKey] = useState(defaultDateRangeKey);
  const [dateRangeDraftKey, setDateRangeDraftKey] = useState(defaultDateRangeKey);
  const [isDateRangeOpen, setIsDateRangeOpen] = useState(false);
  const [isCustomDateRangeOpen, setIsCustomDateRangeOpen] = useState(false);
  const [customDateRange, setCustomDateRange] = useState(
    () => getDateRangeValue(defaultDateRangeKey)
  );
  const [customDateRangeDraft, setCustomDateRangeDraft] = useState(() => getDateRangeValue(defaultDateRangeKey));
  const [customDateRangeMonth, setCustomDateRangeMonth] = useState(
    () => getStartOfMonth(getDateRangeValue(defaultDateRangeKey).start)
  );
  const [entityKeys, setEntityKeys] = useState(
    isSalesByCustomerReport || isCustomerBalanceSummaryReport ? ENTITY_OPTIONS.map((option) => option.key) : []
  );
  ...
  const [customizeDateRangeDraftKey, setCustomizeDateRangeDraftKey] = useState(defaultDateRangeKey);
  ...
  const [customizeCustomDateRangeDraft, setCustomizeCustomDateRangeDraft] = useState(() => getDateRangeValue(defaultDateRangeKey));
  ...
  const [selectedReportColumns, setSelectedReportColumns] = useState(isSalesByCustomerReport ? ["name", "invoice-count", "sales", "sales-with-tax"] : isCustomerBalanceSummaryReport ? ["name", "invoice-amount", "amount-received", "closing-balance"] : ["name", "sales", "sales-with-tax"]);
  const [customizeDraftSelectedColumns, setCustomizeDraftSelectedColumns] = useState(
    isSalesByCustomerReport ? ["name", "invoice-count", "sales", "sales-with-tax"] : isCustomerBalanceSummaryReport ? ["name", "invoice-amount", "amount-received", "closing-balance"] : ["name", "sales", "sales-with-tax"]
  );
  ...
  const customizeDateRangeLabel = DATE_RANGE_OPTIONS.find(
  ...
  const openCustomizeMoreFilterDropdown = (rowId, kind) => {
    setCustomizeMoreFilterDropdown(
      (prev) => prev?.rowId === rowId && prev.kind === kind ? null : { rowId, kind, search: "" }
    );
  };
  ...
  function cancelDateRangeSelection() {
    ...
    setCustomDateRangeMonth(
      getStartOfMonth(
        dateRangeKey === "custom" ? customDateRange.start : getDateRangeValue(dateRangeKey).start
      )
    );
    setIsCustomDateRangeOpen(false);
    setIsDateRangeOpen(false);
  ...
  const filteredCompareWithOptions = useMemo(() => {
    ...
    return COMPARE_WITH_OPTIONS.filter(
      (option) => option.key !== "none" && option.label.toLowerCase().includes(query)
    );
  }, [compareWithSearch]);
  const selectedCustomizeColumns = useMemo(
    () => customizeDraftSelectedColumns.map(
      (key) => getReportColumnOptionForReport(key, reportId)
    ),
  ...
  const filteredCustomizeGroups = useMemo(() => {
    ...
    return REPORT_COLUMN_GROUPS.map((group) => {
      const options = group.options.map((option) => getReportColumnOptionForReport(option.key, reportId)).filter((option) => {
  ...
  const formatReportColumnValue = (key, value) => {
    ...
    const option = getReportColumnOption(key);
    if ((key === "closing-balance" || key === "closing-balance-fcy") && typeof value === "number")
  ...
  const reportColumnTotals = useMemo(
    () => visibleReportColumns.map((option) => {
      ...
      if (option.key === "closing-balance" || option.key === "closing-balance-fcy")
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      ...
      try {
        ...
        const response = isCustomerBalanceSummaryReport ? await reportsAPI.getCustomerBalanceSummary(query) : isBankChargesReport ? await reportsAPI.getBankCharges(query) : isBadDebtsReport ? await reportsAPI.getBadDebts(query) : await reportsAPI.getSalesByCustomer(query);
        if (cancelled) return;
  ...
  const getFilteredComparatorOptions = (query, field) => {
    const normalizedQuery = query.trim().toLowerCase();
    const fieldSpecificOptions = field === "currency" ? MORE_FILTER_COMPARATOR_OPTIONS.filter(
      (option) => ["is-empty", "is-not-empty", "is-in", "is-not-in"].includes(
        option.key
      )
    ) : field === "location" ? MORE_FILTER_COMPARATOR_OPTIONS.filter(
      (option) => ["is-in", "is-not-in"].includes(option.key)
    ) : MORE_FILTER_COMPARATOR_OPTIONS;
    return fieldSpecificOptions.filter(
      (option) => option.label.toLowerCase().includes(normalizedQuery)
    );
  };
  const filteredCompareWithNumberOptions = useMemo(() => {
    const query = compareWithCountSearch.trim().toLowerCase();
    return COMPARE_WITH_NUMBER_OPTIONS.filter(
      (option) => option.includes(query)
    );
  ...
  const handleExportAction = (label) => {
    ...
    const fileBase = `sales-by-customer-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}`;
    const exportHeaders = visibleReportColumns.map((column) => column.label);
    const exportRows = reportRows.map(
      (row) => visibleReportColumns.map(
        (column) => formatReportColumnValue(column.key, row.values[column.key])
      )
    );
  ...
  const openCustomizeColumnsModal = () => {
    ...
    setCustomizeEntityDraftKeys(
      entityKeys.length > 0 ? entityKeys : ENTITY_OPTIONS.map((option) => option.key)
    );
  ...
  const cancelCustomizeColumns = () => {
    ...
    setCustomizeCustomDateRangeMonth(
      getStartOfMonth(
        (dateRangeKey === "custom" ? customDateRange : getDateRangeValue(dateRangeKey)).start
      )
    );
    setIsCustomizeCustomDateRangeOpen(false);
    setCustomizeEntityDraftKeys(
  ...
  const addCustomizeColumn = (key) => {
    setCustomizeDraftSelectedColumns(
  ...
  const removeCustomizeColumn = (key) => {
    if (key === "name") return;
    setCustomizeDraftSelectedColumns(
      (prev) => prev.filter((item) => item !== key)
    );
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
                          ...
                          children: [
                            /* @__PURE__ */ jsxs("div", { className: "mb-2 flex items-center justify-between gap-2 px-1", children: [
                              /* @__PURE__ */ jsx(
                                ...
                                {
                                  type: "button",
                                  onClick: () => setCustomDateRangeMonth(
                              ...
                              /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1", children: [
                                /* @__PURE__ */ jsx(
                                  "select",
                                  {
                                    ...
                                    onChange: (event) => side === "start" ? setLeftCalendarMonth(
                            ...
                            /* @__PURE__ */ jsxs("table", { className: "w-full table-fixed border-collapse text-center text-[11px]", children: [
                              /* @__PURE__ */ jsx("thead", { children: /* @__PURE__ */ jsx("tr", { className: "text-[#1b6f7b]", children: [
                                "Su",
                                "Mo",
                                "Tu",
                                "We",
                                ...
                                return /* @__PURE__ */ jsx(
                                  "td",
                                  {
                                    ...
                                    children: /* @__PURE__ */ jsx(
                                      ...
                                      {
                                        type: "button",
                                        onClick: () => side === "start" ? handleCustomStartDateClick(
                                          day
          ...
          /* @__PURE__ */ jsxs("div", { ref: entityRef, className: "relative inline-flex", children: [
            /* @__PURE__ */ jsxs(
              "button",
              {
                type: "button",
                onClick: () => setIsEntityOpen((prev) => !prev),
                className: `relative inline-flex h-8 w-[184px] items-center overflow-hidden rounded border px-3 pr-12 text-sm text-[#334155] hover:bg-white ${isEntityOpen ? "border-[#1b6f7b] bg-white" : "border-[#cfd6e4] bg-[#f8fafc]"}`,
                "aria-haspopup": "menu",
                "aria-expanded": isEntityOpen,
                children: [
                  /* @__PURE__ */ jsx("span", { className: "shrink-0 whitespace-nowrap", children: "Entities :" }),
                  /* @__PURE__ */ jsx("span", { className: "min-w-0 flex-1 truncate text-left font-medium whitespace-nowrap", children: entityLabel }),
                  /* @__PURE__ */ jsx(
                    ChevronDown,
                    {
                      size: 14,
            ...
            entityKeys.length > 0 ? /* @__PURE__ */ jsx(
              "button",
              {
                type: "button",
                onClick: (event) => {
            ...
            isEntityOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-40 w-[168px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
              ...
              /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: filteredEntityOptions.length > 0 ? filteredEntityOptions.map((option) => {
          ...
          /* @__PURE__ */ jsxs(
            "button",
            {
              ...
              className: `inline-flex h-8 items-center gap-1 rounded border px-3 text-sm text-[#334155] hover:bg-[#f8fafc] ${isMoreFiltersOpen || hasMoreFilters ? "border-[#1b6f7b] bg-white" : "border-[#cfd6e4] bg-white"}`,
              children: [
                /* @__PURE__ */ jsx(Plus, { size: 14, className: "text-[#1b6f7b]" }),
                " More Filters"
              ]
            }
          ),
          isMoreFiltersOpen ? /* @__PURE__ */ jsx(
            "div",
            {
          ...
          isMoreFiltersOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+10px)] z-40 w-[720px] rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
            /* @__PURE__ */ jsxs("div", { className: "px-4 py-4", children: [
              /* @__PURE__ */ jsx("div", { className: "space-y-3", children: moreFilterRows.map((row, index) => {
                ...
                const filteredComparatorOptions = getFilteredComparatorOptions(
                    ...
                    children: [
                          ...
                          /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                            /* @__PURE__ */ jsx(
                              "input",
                              {
                                ...
                                onChange: (event) => setMoreFilterDropdown(
                                  (prev) => prev ? {
                                    ...prev,
                      ...
                      /* @__PURE__ */ jsx("div", { className: "relative", children: valueMode === "none" ? /* @__PURE__ */ jsx("div", { className: "flex h-8 items-center rounded border border-[#e2e8f0] bg-[#f8fafc] px-3 text-sm text-[#94a3b8]", children: "No value needed" }) : valueMode === "text" ? /* @__PURE__ */ jsx(
                        "input",
                        {
                          ...
                          onChange: (event) => {
                            setMoreFilterRows(
                              (prev) => prev.map(
                                (item) => item.id === row.id ? { ...item, value: event.target.value } : item
                              )
                            );
                          },
                      ...
                      /* @__PURE__ */ jsx(
                        "button",
                        {
                          ...
                          onClick: () => {
                            setMoreFilterRows(
                              (prev) => prev.length > 1 ? prev.filter((item) => item.id !== row.id) : prev
                            );
    ...
    /* @__PURE__ */ jsxs("div", { className: "rounded-lg border border-[#e5e7eb] bg-white", children: [
              ...
              compareWithDraftKey !== "none" ? /* @__PURE__ */ jsxs("div", { className: "mt-3", children: [
                ...
                /* @__PURE__ */ jsxs("div", { ref: compareWithCountRef, className: "relative", children: [
                  ...
                  isCompareWithCountOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-[80] w-[168px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                    /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                      /* @__PURE__ */ jsx(
                        "input",
                        {
                          ...
                          onChange: (event) => setCompareWithCountSearch(
                            event.target.value
                          ),
                          placeholder: "Search",
                          className: "h-9 w-full rounded-md border border-[#1b6f7b] bg-white pl-8 pr-3 text-sm text-[#334155] outline-none placeholder:text-[#94a3b8]"
                    ...
                    /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: filteredCompareWithNumberOptions.length > 0 ? filteredCompareWithNumberOptions.map(
                      (option) => {
                ...
                /* @__PURE__ */ jsxs("label", { className: "mt-3 flex items-start gap-2 text-sm text-[#334155]", children: [
                  /* @__PURE__ */ jsx(
                    "input",
                    {
                      ...
                      onChange: (event) => setCompareWithDraftArrangeLatest(
                    ...
                    children: [
                      ...
                      /* @__PURE__ */ jsx(
                        "main",
                        {
                          ...
                          children: customizeReportTab === "general" ? /* @__PURE__ */ jsxs("div", { className: "space-y-6", children: [
                                          ...
                                          /* @__PURE__ */ jsx("div", { className: "w-[165px] border-r border-[#eef2f7]", children: /* @__PURE__ */ jsx("div", { className: "max-h-[280px] overflow-y-auto py-1", children: DATE_RANGE_OPTIONS.map((option) => {
                                            ...
                                            return /* @__PURE__ */ jsxs(
                                              "button",
                                              {
                                                ...
                                                onClick: () => {
                                                  if (option.key === "custom") {
                                                    const currentRange = customizeDateRangeDraftKey === "custom" ? customizeCustomDateRangeDraft : getDateRangeValue(
                                                  ...
                                                  children: [
                                                    /* @__PURE__ */ jsxs("div", { className: "mb-2 flex items-center justify-between gap-2 px-1", children: [
                                                      /* @__PURE__ */ jsx(
                                                        ...
                                                        {
                                                          type: "button",
                                                          onClick: () => setCustomizeCustomDateRangeMonth(
                                                            (prev) => addMonths(prev, -1)
                                                          ),
                                                          className: "inline-flex h-6 w-6 items-center justify-center rounded hover:bg-[#f8fafc]",
                                                          "aria-label": "Previous month",
                                                      ...
                                                      /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1", children: [
                                                        /* @__PURE__ */ jsx(
                                                          ...
                                                          {
                                                            value: month.getMonth(),
                                                            onChange: (event) => side === "start" ? setCustomizeLeftCalendarMonth(
                                                              Number(
                                                                event.target.value
                                                              ),
                                                              month.getFullYear()
                                                            ...
                                                            className: "h-6 rounded border border-[#cfd6e4] bg-white px-1 text-[11px] text-[#334155] outline-none",
                                                            children: MONTH_NAMES.map(
                                                              (monthName, index) => /* @__PURE__ */ jsx(
                                                                "option",
                                                                {
                                                                  value: index,
                                                                  children: monthName
                                                        ...
                                                        /* @__PURE__ */ jsx(
                                                          "select",
                                                          {
                                                            ...
                                                            className: "h-6 rounded border border-[#cfd6e4] bg-white px-1 text-[11px] text-[#334155] outline-none",
                                                            children: CALENDAR_YEAR_OPTIONS.map(
                                                              (year) => /* @__PURE__ */ jsx(
                                                                "option",
                                                                {
                                                                  value: year,
                                                    ...
                                                    /* @__PURE__ */ jsxs("table", { className: "w-full table-fixed border-collapse text-center text-[11px]", children: [
                                                      ...
                                                      /* @__PURE__ */ jsx("tbody", { children: weeks.map(
                                                        (week, weekIndex) => /* @__PURE__ */ jsx(
                                                          "tr",
                                                          {
                                                              ...
                                                              return /* @__PURE__ */ jsx(
                                                                "td",
                                                                {
                                                                  ...
                                                                  children: /* @__PURE__ */ jsx(
                                                                    ...
                                                                    {
                                                                      type: "button",
                                                                      onClick: () => side === "start" ? handleCustomizeCustomStartDateClick(
                                                                        day
                              ...
                              /* @__PURE__ */ jsxs("div", { className: "grid gap-6 md:grid-cols-2", children: [
                                /* @__PURE__ */ jsxs("div", { ref: customizeCompareRef, className: "relative", children: [
                                  ...
                                  isCustomizeCompareOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-50 w-[260px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                                    /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                      /* @__PURE__ */ jsx(
                                        "input",
                                        {
                                          ...
                                          onChange: (event) => setCustomizeCompareSearch(
                                            event.target.value
                                          ),
                                          placeholder: "Search",
                                          className: "h-9 w-full rounded-md border border-[#1b6f7b] bg-white pl-8 pr-3 text-sm text-[#334155] outline-none placeholder:text-[#94a3b8]"
                                    ...
                                    /* @__PURE__ */ jsx("div", { className: "max-h-[170px] overflow-y-auto py-1", children: filteredCustomizeCompareOptions.length > 0 ? filteredCustomizeCompareOptions.map(
                                      (option) => {
                                        const isSelected = compareWithDraftKey === option.key;
                                        return /* @__PURE__ */ jsxs(
                                          "button",
                                ...
                                /* @__PURE__ */ jsx(
                                  "div",
                                  {
                                    ...
                                    className: "relative",
                                    children: compareWithDraftKey !== "none" ? /* @__PURE__ */ jsxs(Fragment, { children: [
                                      ...
                                      isCustomizeCompareCountOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-50 w-[168px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                                        /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                          /* @__PURE__ */ jsx(
                                            "input",
                                            {
                                              ...
                                              onChange: (event) => setCustomizeCompareCountSearch(
                                                event.target.value
                                              ),
                                              placeholder: "Search",
                                              className: "h-9 w-full rounded-md border border-[#1b6f7b] bg-white pl-8 pr-3 text-sm text-[#334155] outline-none placeholder:text-[#94a3b8]"
                                        ...
                                        /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: filteredCustomizeCompareNumberOptions.length > 0 ? filteredCustomizeCompareNumberOptions.map(
                                          (option) => {
                                            const isSelected = String(compareWithDraftCount) === option;
                                            return /* @__PURE__ */ jsxs(
                                              "button",
                              ...
                              /* @__PURE__ */ jsxs("div", { ref: customizeEntityRef, className: "relative", children: [
                                ...
                                /* @__PURE__ */ jsxs(
                                  "button",
                                  {
                                    ...
                                    children: [
                                      /* @__PURE__ */ jsx("span", { className: "truncate", children: getEntitySelectionLabel(
                                ...
                                isCustomizeEntityOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-50 w-[260px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                                  /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                    /* @__PURE__ */ jsx(
                                      "input",
                                      {
                                        ...
                                        onChange: (event) => setCustomizeEntitySearch(
                                          event.target.value
                                        ),
                                        placeholder: "Search",
                                        className: "h-9 w-full rounded-md border border-[#1b6f7b] bg-white pl-8 pr-3 text-sm text-[#334155] outline-none placeholder:text-[#94a3b8]"
                                  ...
                                  /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: ENTITY_OPTIONS.filter(
                                    (option) => option.label.toLowerCase().includes(
                                      customizeEntitySearch.trim().toLowerCase()
                                    )
                                  ).length > 0 ? ENTITY_OPTIONS.filter(
                                    (option) => option.label.toLowerCase().includes(
                                      customizeEntitySearch.trim().toLowerCase()
                                    )
                                  ).map((option) => {
                                    const isSelected = customizeEntityDraftKeys.includes(
                                      option.key
                                    );
                                    return /* @__PURE__ */ jsxs(
                                      "button",
                                      {
                                        ...
                                        onClick: () => {
                                          setCustomizeEntityDraftKeys(
                                            (prev) => prev.includes(option.key) ? prev.filter(
                                              (key) => key !== option.key
                                            ) : [...prev, option.key]
                                          );
                                        },
                                        className: `flex w-full items-center gap-2 px-4 py-2 text-left text-sm ${isSelected ? "bg-[#f1f5f9] font-medium text-[#0f172a]" : "text-[#334155] hover:bg-[#f8fafc]"}`,
                                        children: [
                                          /* @__PURE__ */ jsx("span", { className: "inline-flex h-4 w-4 items-center justify-center rounded border border-[#c7d0de] bg-white", children: isSelected ? /* @__PURE__ */ jsx(
                                            Check,
                                            {
                                              size: 12,
                                              className: "text-[#0f172a]"
                            ...
                            /* @__PURE__ */ jsxs("section", { ref: customizeMoreFiltersRef, children: [
                              ...
                              customizeMoreFilterRows.length > 0 ? /* @__PURE__ */ jsx("div", { className: "mt-4 space-y-3", children: customizeMoreFilterRows.map((row, index) => {
                                ...
                                const valueOptions = row.field ? MORE_FILTER_VALUE_OPTIONS[row.field] : [];
                                const valueMode = row.comparator && MORE_FILTER_NO_VALUE_COMPARATORS.includes(
                                  row.comparator
                                ) ? "none" : row.comparator && MORE_FILTER_TEXT_COMPARATORS.includes(
                                  row.comparator
                                ) ? "text" : "dropdown";
                                ...
                                const valueLabel = valueMode === "text" ? row.value || "Enter a value" : row.value ? getMoreFilterValueLabel(
                                  row.field,
                                  row.value
                                    ...
                                    children: [
                                      ...
                                      /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                        /* @__PURE__ */ jsxs(
                                          ...
                                          {
                                            type: "button",
                                            onClick: () => openCustomizeMoreFilterDropdown(
                                              row.id,
                                              "field"
                                            ),
                                            className: `relative flex h-8 w-full items-center overflow-hidden rounded border px-3 pr-10 text-sm text-[#334155] outline-none ${fieldMenuOpen ? "border-[#1b6f7b] bg-white" : "border-[#cfd6e4] bg-white hover:bg-[#f8fafc]"}`,
                                        ...
                                        fieldMenuOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-50 w-[240px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                                          /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                            /* @__PURE__ */ jsx(
                                              ...
                                              {
                                                value: fieldMenuSearch,
                                                onChange: (event) => setCustomizeMoreFilterDropdown(
                                                  (prev) => prev ? {
                                                    ...prev,
                                                    search: event.target.value
                                                  } : prev
                                          ...
                                          /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: filteredFieldGroups.length > 0 ? filteredFieldGroups.map((group) => /* @__PURE__ */ jsxs("div", { children: [
                                            /* @__PURE__ */ jsx("div", { className: "px-3 py-2 text-sm font-semibold text-[#475569]", children: group.label }),
                                            /* @__PURE__ */ jsx("div", { className: "pb-1", children: group.options.map(
                                              (option) => {
                                                const isSelected = row.field === option.key;
                                                return /* @__PURE__ */ jsxs(
                                                  "button",
                                      ...
                                      /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                        ...
                                        comparatorMenuOpen ? /* @__PURE__ */ jsxs("div", { className: "absolute left-0 top-[calc(100%+6px)] z-50 w-[168px] overflow-hidden rounded-lg border border-[#d7dce7] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]", children: [
                                          ...
                                          /* @__PURE__ */ jsx("div", { className: "max-h-[220px] overflow-y-auto py-1", children: filteredComparatorOptions.length > 0 ? filteredComparatorOptions.map(
                                            (option) => {
                                              const isSelected = row.comparator === option.key;
                                              return /* @__PURE__ */ jsxs(
                                                "button",
                                      ...
                                      /* @__PURE__ */ jsx(
                                        "button",
                                        {
                                          ...
                                          onClick: () => {
                                            setCustomizeMoreFilterRows(
                                              (prev) => prev.length > 1 ? prev.filter(
                                                (item) => item.id !== row.id
                                              ) : prev
                                            );
                                ...
                                /* @__PURE__ */ jsx("div", { className: "border-b border-[#eef2f7] p-2", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
                                  /* @__PURE__ */ jsx(
                                    "input",
                                    {
                                      ...
                                      onChange: (event) => setCustomizeColumnsSearch(
                                        event.target.value
                                      ),
                                      placeholder: "Search",
                                ...
                                /* @__PURE__ */ jsx("div", { className: "max-h-[440px] overflow-y-auto py-1", children: filteredCustomizeGroups.length > 0 ? filteredCustomizeGroups.map((group) => /* @__PURE__ */ jsxs("div", { children: [
                                  ...
                                  /* @__PURE__ */ jsx("div", { className: "pb-1", children: group.options.map((option) => {
                                    ...
                                    return /* @__PURE__ */ jsx(
                                      ...
                                      {
                                        type: "button",
                                        onClick: () => setCustomizeActiveAvailableColumn(
                                          option.key
                                        ),
                            ...
                            /* @__PURE__ */ jsxs("div", { children: [
                              ...
                              /* @__PURE__ */ jsx("div", { className: "rounded-lg border border-[#d7dce7] bg-white", children: /* @__PURE__ */ jsx("div", { className: "max-h-[440px] overflow-y-auto p-3", children: /* @__PURE__ */ jsx("div", { className: "space-y-1", children: selectedCustomizeColumns.map((option) => /* @__PURE__ */ jsxs(
                                "div",
                                {
      ...
      /* @__PURE__ */ jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxs("table", { className: "w-full min-w-[720px] border-collapse", children: [
        ...
        /* @__PURE__ */ jsx("tbody", { children: isReportLoading ? /* @__PURE__ */ jsx("tr", { className: "border-b border-[#eef2f7]", children: /* @__PURE__ */ jsx(
          "td",
          {
        ...
        ) }) : reportError ? /* @__PURE__ */ jsx("tr", { className: "border-b border-[#eef2f7]", children: /* @__PURE__ */ jsx(
          "td",
          {
        ...
        ) }) : reportRows.length === 0 ? /* @__PURE__ */ jsx("tr", { className: "border-b border-[#eef2f7]", children: /* @__PURE__ */ jsx(
          "td",
          {
            className: "px-4 py-8 text-center text-sm text-[#64748b]",
            colSpan: visibleReportColumns.length,
            children: "No report rows found for the selected filters."
          }
        ) }) : /* @__PURE__ */ jsxs(Fragment, { children: [
          reportRows.map((row, index) => /* @__PURE__ */ jsx(
            "tr",
            {
              className: "border-b border-[#eef2f7]",
              children: visibleReportColumns.map((column) => /* @__PURE__ */ jsx(
                "td",
                {
                  className: `px-4 py-3 text-sm ${column.kind === "text" ? "text-left" : "text-center"} ${column.key === "name" ? "font-medium text-[#2563eb]" : "text-[#2563eb]"}`,
                  children: formatReportColumnValue(
                    column.key,
...
function ReportDetailPage() {
  ...
  if (report.id === "sales-by-customer" || report.id === "customer-balance-summary" || report.id === "bad-debts" || report.id === "bank-charges") {
  ...
  if (report.id === "ar-aging-summary") {
    return /* @__PURE__ */ jsx(ARAgingSummaryReportPage, {});
  }
  if (report.id === "ar-aging-details") {
    return /* @__PURE__ */ jsx(ARAgingDetailsReportPage, {});
  }
  if (report.id === "invoice-details") {
    return /* @__PURE__ */ jsx(InvoiceDetailsReportPage, {});
  }
  if (report.id === "quote-details") {
    return /* @__PURE__ */ jsx(QuoteDetailsReportPage, {});
  }
  return /* @__PURE__ */ jsxs("div", { className: "relative min-h-[calc(100vh-64px)] pt-3", children: [
    /* @__PURE__ */ jsx(
      ReportsDrawer,
            ...
            /* @__PURE__ */ jsx("div", { className: "mt-3 overflow-x-auto", children: /* @__PURE__ */ jsxs("table", { className: "w-full min-w-[720px] border-collapse", children: [
              ...
              /* @__PURE__ */ jsx("tbody", { children: REPORT_FUNCTION_LABELS.map((item) => {
                ...
                return /* @__PURE__ */ jsxs("tr", { className: "border-b border-[#eef2f7]", children: [
                  ...
                  /* @__PURE__ */ jsx("td", { className: "px-3 py-2 text-sm", children: /* @__PURE__ */ jsx(
                    "span",
                    {
                      className: supported ? "rounded bg-[#dcfce7] px-2 py-1 text-xs font-semibold text-[#166534]" : "rounded bg-[#f1f5f9] px-2 py-1 text-xs font-semibold text-[#475569]",
                      children: supported ? "Supported" : "Not Supported"
                    }
                  ) })
                ] }, item.key);
              }) })
