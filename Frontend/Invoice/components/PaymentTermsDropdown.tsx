...
export function PaymentTermsDropdown({
  ...
  const selected = customTerms.find((term) => term.value === value);
  ...
  const filteredTerms = useMemo(() => {
    ...
    if (!query) return customTerms;
    return customTerms.filter((term) =>
      [term.label, term.value, String(term.days ?? "")]
        .join(" ")
        .toLowerCase()
        .includes(query)
    );
  }, [customTerms, searchQuery]);
  ...
  return (
    <div ref={rootRef} className="relative w-full">
      <button
        ...
        className="flex h-9 w-full items-center justify-between rounded-md border border-[#3f83f8] bg-white px-3 text-sm text-gray-700"
      >
        <span className="truncate">{selected?.label || "Select payment term"}</span>
        <ChevronDown
          size={16}
          className={`text-[#3f83f8] transition-transform ${isOpen ? "rotate-180" : ""}`}
        />
      </button>

      {isOpen && (
        <div className="absolute left-0 top-full z-50 mt-2 w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.14)]">
