...
export default function ImportSalesReceipts() {
    ...
    const headerAliases: Record<string, string[]> = {
        "Sales Receipt#": ["sales receipt", "receipt number", "receipt #", "sr number", "sr #", "number"],
        "Receipt Date": ["date", "sales receipt date", "receipt dt"],
        "Customer Name": ["customer", "client", "client name", "company", "company name", "display name"],
        "Reference": ["reference number", "payment reference", "ref"],
        "Salesperson": ["sales person", "owner"],
        "Payment Mode": ["payment method", "mode", "method"],
        "Deposit To": ["deposit account", "deposit to account", "bank account", "cash account"],
        ...
        "Discount": ["discount amount", "transaction discount", "line discount"],
        "Discount Type": ["discount mode", "discount unit", "discount kind"],
        "Tax": ["tax amount", "vat", "sales tax"],
        "Shipping Charges": ["shipping", "shipping charge", "delivery charge", "freight"],
        "Shipping Charge Tax": ["shipping tax", "shipping vat", "shipping charge vat"],
        "Adjustment": ["adjustments", "rounding adjustment"],
        "Total": ["grand total", "amount due", "receipt total"],
        "Description": ["details", "notes", "memo"],
        "Currency": ["currency code"]
    };

    const resolveMappedHeader = (fieldName: string, headersList = importedFileHeaders): string => {
        ...
        if (currentStep === "configure") {
            ...
            if (selectedFile) {
                try {
                    ...
                    const requiredFields = ["Sales Receipt#", "Customer Name", "Receipt Date", "Item Name"];
                    let unmappedCount = 0;
    ...
    const downloadSampleFile = (type: "csv" | "xls") => {
        ...
        link.href = url;
        link.download = `sales-receipts-import-sample.${extension}`;
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            // Import each row as a sales receipt
            let importedCount = 0;
            let skippedCount = 0;
            const errors: string[] = [];
            const customers = await getCustomers();
            const existingReceipts = await getSalesReceipts();
            const items = await getItemsFromAPI();

            const accountsResponse: any = await chartOfAccountsAPI.getAccounts({ limit: 1000 });
            const allAccounts = Array.isArray(accountsResponse?.data)
            ...
            const depositAccounts = allAccounts.filter((acc: any) => {
                const accountType = String(acc.account_type || acc.accountType || "").toLowerCase();
                const accountName = String(acc.accountName || acc.name || "").toLowerCase();
                return (
            ...
            const fallbackAccount = depositAccounts[0] || allAccounts[0];
            const defaultDepositTo = String(
                fallbackAccount?.id ||
                fallbackAccount?._id ||
            ...
            const normalizeDiscountType = (rawType: any, rawDiscountValue: any): "percent" | "amount" => {
                ...
                if (!normalized) {
                    return String(rawDiscountValue || "").includes("%") ? "percent" : "percent";
                }
                if (
                    normalized.includes("%") ||
            ...
            const findCustomerId = (customerName: string): string => {
            ...
            const findItem = (itemValue: string, fallbackValue = "", rowData: Record<string, any> = {}): any => {
                ...
                const exactNeedles = lookupValues.map((value) => value.toLowerCase());
                const normalizedNeedles = lookupValues
                    .map((value) => normalizeLookupValue(value))
                    .filter(Boolean);

                const getItemCandidates = (item: any) => [
                    item?.id,
                    item?._id,
                    item?.name,
                    item?.itemName,
                ...
                // 1) Exact raw match
                const exactRawMatch = items.find((item: any) => {
                    const candidates = getItemCandidates(item).map((value: string) => value.toLowerCase());
                    return exactNeedles.some((needle) => candidates.includes(needle));
                });
                if (exactRawMatch) return exactRawMatch;

                // 2) Exact normalized match
                const exactNormalizedMatch = items.find((item: any) => {
                    const candidates = getItemCandidates(item).map((value: string) => normalizeLookupValue(value));
                    return normalizedNeedles.some((needle) => candidates.includes(needle));
                });
                if (exactNormalizedMatch) return exactNormalizedMatch;

                // 3) Partial normalized match for longer needles
                const partialNeedles = normalizedNeedles.filter((needle) => needle.length >= 3);
                if (partialNeedles.length) {
                    const partialMatch = items.find((item: any) => {
                        const candidates = getItemCandidates(item).map((value: string) => normalizeLookupValue(value));
                        return partialNeedles.some((needle) =>
                ...
                // 4) Try finding item from row cells that look like item-related columns
                const itemLikeRowValues = Object.entries(rowData || {})
                    .filter(([key, value]) => {
                        ...
                        return looksLikeItemColumn && hasValue;
                    })
                    .map(([, value]) => String(value || "").trim());

                if (itemLikeRowValues.length) {
                    const rowNeedles = itemLikeRowValues.map((value) => value.toLowerCase());
                    const rowNormalizedNeedles = itemLikeRowValues.map((value) => normalizeLookupValue(value)).filter(Boolean);

                    const rowExactMatch = items.find((item: any) => {
                        const candidates = getItemCandidates(item).map((value: string) => value.toLowerCase());
                        return rowNeedles.some((needle) => candidates.includes(needle));
                    });
                    if (rowExactMatch) return rowExactMatch;

                    const rowNormalizedMatch = items.find((item: any) => {
                        const candidates = getItemCandidates(item).map((value: string) => normalizeLookupValue(value));
                        return rowNormalizedNeedles.some((needle) => candidates.includes(needle));
                    });
                    if (rowNormalizedMatch) return rowNormalizedMatch;
                }

                // 5) Last safe fallback: if organization has only one item, use it.
                if (items.length === 1) return items[0];

                return null;
            };

            const resolveDepositToAccount = (rawDepositValue: string): string => {
                const value = String(rawDepositValue || "").trim();
                if (!value) return defaultDepositTo;

                const normalized = value.toLowerCase();
                const account = (depositAccounts.length > 0 ? depositAccounts : allAccounts).find((acc: any) => {
                    const candidates = [
                        acc?.id,
                        acc?._id,
                        acc?.accountName,
                        acc?.name
                    ].map((candidate: any) => String(candidate || "").trim().toLowerCase()).filter(Boolean);
                    return candidates.includes(normalized) || candidates.some((candidate: string) => candidate.includes(normalized));
                });
            ...
            const getGeneratedReceiptNumber = async () => {
                try {
                    const response = await salesReceiptsAPI.getNextNumber();
                    const generated = String(response?.data?.nextNumber || "").trim();
            ...
            const ensureFallbackImportItem = async () => {
                if (fallbackImportItem) return fallbackImportItem;

                const existing = items.find((item: any) => {
                    const itemName = String(item?.name || item?.itemName || "").trim().toLowerCase();
                    return itemName === "imported sales receipt item" || itemName === "imported item";
                });
                if (existing) {
                    fallbackImportItem = existing;
                    return fallbackImportItem;
                }

                try {
                    const createdResponse = await itemsAPI.create({
                        name: "Imported Sales Receipt Item",
                        type: "Service",
                        sellingPrice: 0,
                        trackInventory: false,
                        isActive: true
                    });
                    const createdItem = createdResponse?.data || null;
                    if (createdItem) {
                        items.push(createdItem);
                        fallbackImportItem = createdItem;
                        return fallbackImportItem;
                    }
                } catch (error) {
                    console.warn("Failed to auto-create fallback import item:", error);
                }
            ...
            for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                ...
                try {
                    ...
                    // Get required fields
                    const receiptNumber = getValue("Sales Receipt#");
                    const customerName = getValue("Customer Name");
                    const receiptDate = getValue("Receipt Date");

                    // Skip if required customer field is missing
                    if (!customerName) {
                        skippedCount++;
                    ...
                    const parsedDate = parseDateValue(receiptDate, new Date());
                    ...
                    let matchedItem = findItem(itemName, description, row);
                    if (!matchedItem) {
                        const hasItemText = Boolean(String(itemName || "").trim() || String(description || "").trim());

                        if (!hasItemText) {
                            const fallbackItem = await ensureFallbackImportItem();
                            if (fallbackItem) {
                        ...
                        if (!matchedItem) {
                            skippedCount++;
                            errors.push(`Row ${rowIndex + 1}: Missing item. Map the "Item Name" column or include an item name/sku.`);
                            continue;
                        }
                    }

                    const parsedQuantity = parseNumber(quantity, 1);
                    const parsedRate = parseNumber(rate, parseNumber(matchedItem?.sellingPrice || matchedItem?.rate, 0));
                    const parsedAmount = parseNumber(amount, parsedQuantity * parsedRate);

                    const discountRawValue = getValue("Discount");
                    const discountTypeRawValue = getValue("Discount Type");
                    const taxRawValue = getValue("Tax");
                    const shippingRawValue = getValue("Shipping Charges");
                    const shippingChargeTaxValue = getValue("Shipping Charge Tax");
                    const adjustmentRawValue = getValue("Adjustment");
                    const totalRawValue = getValue("Total");

                    const parsedSubtotal = Math.max(0, parsedAmount);
                    const parsedDiscount = Math.max(0, parseNumber(discountRawValue, 0));
                    const parsedDiscountType = normalizeDiscountType(discountTypeRawValue, discountRawValue);
                    const parsedDiscountAmount = parsedDiscountType === "percent"
                        ? (parsedSubtotal * parsedDiscount / 100)
                        : parsedDiscount;
                    const parsedTax = Math.max(0, parseNumber(taxRawValue, 0));
                    const parsedShippingCharges = parseNumber(shippingRawValue, 0);
                    const parsedAdjustment = parseNumber(adjustmentRawValue, 0);
                    const computedTotal = Math.max(
                        0,
                        parsedSubtotal + parsedTax - parsedDiscountAmount + parsedShippingCharges + parsedAdjustment
                    );
                    const parsedTotal = parseNumber(totalRawValue, computedTotal);

                    const itemsPayload = [{
                        item: String(matchedItem?.id || matchedItem?._id || "").trim(),
                    ...
                    const resolvedReceiptNumber = (autoGenerateNumbers || !receiptNumber)
                        ? await getGeneratedReceiptNumber()
                        : String(receiptNumber).trim();

                    if (!resolvedReceiptNumber) {
                        skippedCount++;
                        errors.push(`Row ${rowIndex + 1}: Could not generate receipt number`);
                        continue;
                    }

                    const depositToAccount = resolveDepositToAccount(getValue("Deposit To"));
                    if (!depositToAccount) {
                        skippedCount++;
                        errors.push(`Row ${rowIndex + 1}: Deposit To Account is required`);
                        continue;
                    }

                    const paymentMethod = normalizePaymentMethod(getValue("Payment Mode"));
                    ...
                    const receiptData = {
                        ...
                        shippingChargeTax: String(shippingChargeTaxValue || "").trim(),
                    ...
                    console.log("Importing sales receipt:", receiptData);

                    // Check for duplicates
                    if (duplicateHandling === "skip") {
                        const isDuplicate = existingReceipts.some(r =>
                            r.receiptNumber === receiptData.receiptNumber && receiptData.receiptNumber && !autoGenerateNumbers
                        );
                        if (isDuplicate) {
                            skippedCount++;
                            continue;
                        ...
                        const existingReceipt = existingReceipts.find(r =>
                            r.receiptNumber === receiptData.receiptNumber && receiptData.receiptNumber && !autoGenerateNumbers
                        );
                        if (existingReceipt) {
                            await updateSalesReceipt(existingReceipt.id, { ...existingReceipt, ...receiptData, id: existingReceipt.id });
                            importedCount++;
                            continue;
                        }
                    }
                    ...
                    console.error("Error importing sales receipt:", error);
                    skippedCount++;
            ...
            if (importedCount > 0) {
                alert(`Successfully imported ${importedCount} sales receipt(s).${skippedCount > 0 ? ` ${skippedCount} record(s) skipped.` : ''}`);
                navigate("/sales/sales-receipts");
            } else {
                alert(`No sales receipts were imported.${errors.length > 0 ? ` ${errors.slice(0, 3).join(" ")}` : ""}`);
            }
        } catch (error) {
            console.error("Error importing sales receipts:", error);
            alert("Error importing sales receipts. Please check the file format and try again.");
        }
    };
            ...
            <div className="w-full max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 p-6">
                    <div className="flex items-center justify-between">
                        <h1 className="text-2xl font-bold text-gray-900">
                            {currentStep === "configure" ? "Sales Receipts - Select File" : currentStep === "mapFields" ? "Map Fields" : "Preview"}
                        </h1>
                                ...
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs text-gray-600 mt-1">
                                            Retains the sales receipts in Taban Books and does not import the duplicates in the import file.
                                        </div>
                                    </div>
                                </label>
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    />
                                    <div className="flex-1">
                                        <div className="text-sm font-medium text-gray-900">Overwrite sales receipts</div>
                                        <div className="text-xs text-gray-600 mt-1">
                                            Imports the duplicates in the import file and overwrites the existing sales receipts in Taban Books.
                                        </div>
                                    </div>
                                </label>
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    />
                                    <div className="flex-1">
                                        <div className="text-sm font-medium text-gray-900">Add duplicates as new sales receipts</div>
                                        <div className="text-xs text-gray-600 mt-1">
                                            Imports the duplicates in the import file and adds them as new sales receipts in Taban Books.
                                        </div>
                                    </div>
                                </label>
                            </div>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-gray-600 mt-1">
                                        Receipt numbers will be generated automatically according to your settings. Any numbers in the import file will be ignored.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {currentStep === "mapFields" && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                        ...
                        <div className="space-y-4">
                            ...
                            {["Sales Receipt#", "Receipt Date", "Customer Name", "Reference", "Salesperson", "Payment Mode", "Deposit To", "Item Name", "Quantity", "Rate", "Amount", "Discount", "Discount Type", "Tax", "Shipping Charges", "Shipping Charge Tax", "Adjustment", "Total", "Description", "Currency"].map((field) => (
                                <div key={field} className="grid grid-cols-12 gap-4 items-center">
                                    <div className="col-span-4 text-sm font-medium text-gray-700">
                                        {field} <span className="text-red-500">{["Sales Receipt#", "Customer Name", "Receipt Date", "Item Name"].includes(field) && "*"}</span>
                                    </div>
                                    <div className="col-span-4">
                                        <div className="relative">
                                            <select
                                                ...
                                                value={fieldMappings[field] || resolveMappedHeader(field)}
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                        ...
                        <div className="grid grid-cols-3 gap-6 mb-8">
                            <div className="border border-green-200 bg-green-50 rounded-lg p-4 text-center">
                                ...
                                <div className="text-sm font-medium text-green-800">Sales Receipts Ready</div>
                            </div>
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
                            ...
                            <div className="text-sm text-blue-800">
                                ...
                                <p>Click the "Import" button below to verify and import your sales receipts data. Make sure all required fields are mapped correctly.</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-white border-t border-gray-200 p-6 flex items-center justify-between sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                    <button
                        ...
                        onClick={currentStep === "configure" ? handleClose : handlePrevious}
                    >
