...
import ImportQuotesConfigureStep from "./ImportQuotesConfigureStep";
import ImportQuotesMapFieldsStep from "./ImportQuotesMapFieldsStep";
import ImportQuotesPreviewStep from "./ImportQuotesPreviewStep";
import ImportQuotesCloudPickerModal from "./ImportQuotesCloudPickerModal";
import ImportQuotesDocumentsModal from "./ImportQuotesDocumentsModal";
...
export default function ImportQuotesView({ controller }: Props) {
  return (
    <div className="w-full min-h-screen bg-gray-50">
      <div className="w-full max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 p-6">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-semibold text-gray-900">
              {controller.currentStep === "configure" ? "Quotes - Select File" : controller.currentStep === "mapFields" ? "Map Fields" : "Preview"}
            </h1>
            <button
        ...
        {controller.currentStep === "configure" && <ImportQuotesConfigureStep controller={controller} />}
        {controller.currentStep === "mapFields" && <ImportQuotesMapFieldsStep controller={controller} />}
        {controller.currentStep === "preview" && <ImportQuotesPreviewStep controller={controller} />}
      </div>

      <ImportQuotesCloudPickerModal controller={controller} />
      <ImportQuotesDocumentsModal controller={controller} />
    </div>
  );
}
