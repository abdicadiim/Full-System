...
import { Info, Phone, Smartphone, Upload, X, Search, ChevronDown, Check, Globe, File, CheckCircle, Plus, MoreVertical, Folder, Cloud, Box, Layers, HardDrive, Settings, Paperclip, FileText, CreditCard, ChevronUp, Square, Grid3x3, RefreshCw, LayoutGrid, Loader2, Facebook, Link2, Mail } from "lucide-react";
...
const MAX_CUSTOMER_DOCUMENTS = 3;
...
export default function NewCustomer() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (formData.documents.length + files.length > MAX_CUSTOMER_DOCUMENTS) {
      toast.error(`You can upload a maximum of ${MAX_CUSTOMER_DOCUMENTS} files`);
      return;
    }
  ...
  const allCountryNames = Array.from(
    new Set([...countryOptions.map((country) => country.name), ...Object.keys(countryData)])
  ...
  const getCountryIsoByName = (countryName: string) => {
  ...
  const getStatesByCountryName = (countryName: string) => {
    ...
    const libraryStates = countryIso
      ...
      : [];
    return libraryStates.length > 0 ? libraryStates : getFallbackStatesByCountryName(countryName);
  };
  ...
  const currencyDisplay =
    (!isEditMode && baseCurrency && baseCurrency.code ? baseCurrency : selectedCurrency) ||
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-gray-50 overflow-x-hidden">
      {/* Header */}
      <div className="border-b border-gray-200 bg-white px-4 sm:px-6 py-3 flex items-center justify-between gap-4 flex-shrink-0">
        <h1 className="text-lg sm:text-xl font-semibold text-gray-900 m-0 whitespace-nowrap">{isEditMode ? "Edit Customer" : "New Customer"}</h1>
        <div className="flex items-center gap-2">
          ...
          {isEmbeddedQuickAction && (
            <button
              type="button"
              onClick={handleCancel}
              className="w-7 h-7 flex items-center justify-center text-red-500 hover:text-red-600 rounded ml-1"
          ...
          <div className="w-full max-w-4xl px-4 sm:px-6 py-5 sm:py-8 overflow-x-hidden">
            ...
            <div>
              <div className="space-y-6">
                ...
                {/* Customer Language */}
                <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-start mb-10">
                  ...
                  <div className="w-full max-w-md">
                    <SearchableDropdown
                      ...
                      onChange={(value) => setFormData((prev: typeof formData) => ({ ...prev, customerLanguage: value }))}
                      placeholder="English"
                      accentColor="#156372"
                    ...
                    <div className="mt-4 flex items-start gap-2.5">
                      <input
                        type="checkbox"
                        name="isTrackedForMoss"
                        checked={!!formData.isTrackedForMoss}
                        onChange={handleChange}
                        className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[#156372] focus:ring-[#156372]"
                      />
                      <div className="min-w-0">
                        <div className="text-[13px] text-gray-800">
                          Track VAT MOSS, OSS, IOSS, or sale of digital services for this customer
                        </div>
                        <div className="mt-1 text-[12px] text-gray-500 leading-snug">
                          Enable this option if this is an overseas customer or they are in an EU member state so that you
                          can record and track VAT MOSS, OSS, IOSS, or digital services export using reports.
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Reporting Tags */}
                {availableReportingTags.filter((tag) => String(tag?.name || "").trim().toLowerCase() !== "sdv").length > 0 && (
                  <div className="space-y-4 mb-10">
                    {availableReportingTags
                      .filter((tag) => String(tag?.name || "").trim().toLowerCase() !== "sdv")
                      .map((tag) => {
              ...
              <div className="mt-6">
                {activeTab === "other-details" && (
                  <div className="space-y-6">
                      ...
                      <div className="w-full max-w-md">
                        <div className="relative">
                          <select
                            ...
                            disabled
                            className="w-full appearance-none px-3 py-1.5 pr-10 border border-gray-300 rounded text-[13px] text-gray-700 bg-white focus:outline-none focus:ring-1 focus:ring-[#156372] focus:border-[#156372] transition-colors disabled:bg-white disabled:text-gray-700 disabled:cursor-not-allowed disabled:opacity-100"
                          >
                        ...
                        <div className="mt-2 flex items-center gap-1.5 text-[12px] text-gray-500 leading-snug">
                          <span>
                            Currency cannot be edited as multi-currency handling is unavailable in Zoho Invoice.
                          </span>
                          <HelpTooltip text="Currency is locked to the base currency for customers.">
                            <Info size={14} className="text-gray-400 cursor-help" />
                          </HelpTooltip>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
                      ...
                      <div className="w-full max-w-md relative">
                        <select
                          ...
                          onChange={handleChange}
                          className="w-full appearance-none px-3 py-1.5 pr-10 border border-gray-300 rounded text-[13px] text-gray-700 bg-white focus:outline-none focus:ring-1 focus:ring-[#156372] focus:border-[#156372] transition-colors"
                        >
                          <option value="">Select a Price List</option>
                          {priceLists.map((p) => (
                            <option key={p.id} value={p.id}>{p.name} {p.currency !== "-" ? `(${p.currency})` : ""}</option>
                          ))}
                        </select>
                        <ChevronDown
                          size={14}
                    ...
                    {/* Portal Language */}
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
                      <label className="text-[13px] font-medium text-gray-700">Portal Language</label>
                      <div className="w-full max-w-md relative">
                        <select
                          ...
                          className="w-full appearance-none px-3 py-1.5 pr-10 border border-gray-300 rounded text-[13px] text-gray-700 bg-white focus:outline-none focus:ring-1 focus:ring-[#156372] focus:border-[#156372] transition-colors"
                        >
                          {customerLanguageOptions.map((opt) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-start" ref={uploadDropdownRef}>
                      ...
                      <div className="w-full max-w-md">
                        ...
                        <p className="mt-1.5 text-[11px] text-gray-500">
                          You can upload a maximum of {MAX_CUSTOMER_DOCUMENTS} files, 10MB each
                        </p>
                      ...
                      <div className="space-y-6 pt-6 border-t border-gray-100 mt-6 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
                          ...
                          <div className="w-full max-w-md">
                            ...
                            <div className="mt-1 text-[12px] text-gray-500">https://x.com/</div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
                          ...
                          <div className="w-full max-w-md flex">
                            <div className="flex items-center justify-center px-3 py-1.5 border border-r-0 border-gray-300 rounded-l bg-white min-w-[44px]">
                              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#00aff0] text-white text-[12px] font-bold leading-none select-none">
                                S
                              </span>
                            </div>
                            <input
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
                          ...
                          <div className="w-full max-w-md">
                            ...
                            <div className="mt-1 text-[12px] text-gray-500">http://www.facebook.com/</div>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="mt-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center mb-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center mb-5" ref={billingCountryDropdownRef}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-start mb-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center mb-5" ref={billingStateDropdownRef}>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center mb-5" ref={shippingCountryDropdownRef}>
                          <label className="text-[13px] font-medium text-gray-700">Country/Region</label>
                          <div className="w-full max-w-sm relative">
                            <button
                              type="button"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center mb-5" ref={shippingStateDropdownRef}>
                          <label className="text-[13px] font-medium text-gray-700">State</label>
                          <div className="w-full max-w-sm relative">
                            <button
                              type="button"
                    ...
                    {Object.keys(formData.customFields || {}).length === 0 ? (
                      <div className="min-h-[260px] flex items-center justify-center">
                        <p className="max-w-2xl text-center text-[13px] text-gray-500 leading-relaxed">
                          ...
                          <span className="text-gray-400">➜</span>{" "}
                          <span className="italic">Customers</span>. You can also refine the address format of your Customers from there.
                        </p>
                      </div>
                    ) : (
              ...
              {/* Action Buttons */}
              <div className="sticky bottom-0 z-40 -mx-4 sm:-mx-6 border-t border-gray-200 bg-white/95 backdrop-blur px-4 sm:px-6 py-4">
                <button
                  type="button"
                  onClick={handleSave}
                  disabled={isSaving}
