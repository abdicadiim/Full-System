...
export default function NewCustomer() {
  ...
  const cachedRouteCustomer = React.useMemo(() => {
    ...
    try {
      const raw =
        sessionStorage.getItem(`${CUSTOMER_EDIT_PRELOAD_PREFIX}${String(id).trim()}`) ||
        sessionStorage.getItem(`${LEGACY_CUSTOMER_EDIT_PRELOAD_PREFIX}${String(id).trim()}`);
  ...
  useEffect(() => {
    if (preloadedEditCustomer) {
      ...
      if (isEditMode && id && typeof window !== "undefined") {
        try {
          const storageKey = String(id).trim();
          sessionStorage.removeItem(`${CUSTOMER_EDIT_PRELOAD_PREFIX}${storageKey}`);
          sessionStorage.removeItem(`${LEGACY_CUSTOMER_EDIT_PRELOAD_PREFIX}${storageKey}`);
        } catch {
        }
      }
    }
    ...
    try {
      ...
      if (savedCustomer) {
        ...
        if (isEditMode && savedCustomerId && typeof window !== "undefined") {
          try {
            sessionStorage.setItem(`${CUSTOMER_EDIT_PRELOAD_PREFIX}${savedCustomerId}`, JSON.stringify(normalizedSavedCustomer));
            sessionStorage.removeItem(`${LEGACY_CUSTOMER_EDIT_PRELOAD_PREFIX}${savedCustomerId}`);
          } catch {
          }
        }
