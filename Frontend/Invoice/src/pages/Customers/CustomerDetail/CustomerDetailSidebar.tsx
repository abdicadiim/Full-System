...
export default function CustomerDetailSidebar(args: any) {
  ...
  const accentColor = String(settings?.theme?.accentColor || settings?.theme?.primaryColor || "rgb(21, 99, 114)").trim();

  return (
    <div className="flex h-full min-h-0 w-64 flex-col overflow-visible border-r border-gray-200 bg-white">
      {selectedCustomers.length > 0 ? (
        <div className="border-b border-gray-200 bg-gray-50/70 p-1.5">
          <div className="flex flex-nowrap items-center justify-between gap-1.5 overflow-hidden rounded-xl border border-gray-200 bg-white px-2 py-2 shadow-[0_1px_2px_rgba(15,23,42,0.06)]">
            <div className="flex min-w-0 flex-1 items-center gap-1.5">
              <input type="checkbox" checked={selectedCustomers.length === customers.length} onChange={handleSelectAllCustomers} className="h-4 w-4 cursor-pointer shrink-0" />
              <div className="relative shrink-0" ref={bulkActionsDropdownRef}>
                <button
                  className="flex w-[84px] shrink-0 cursor-pointer items-center justify-between gap-1 rounded-md border border-gray-300 bg-white px-2 py-1.5 text-[12px] leading-none whitespace-nowrap text-gray-700 transition-colors hover:bg-gray-50"
            ...
            <div className="ml-auto flex shrink-0 items-center gap-1 whitespace-nowrap">
              <span
                className="flex h-6 min-w-[24px] shrink-0 items-center justify-center rounded-full px-2 text-xs font-semibold leading-none text-white"
                style={{ backgroundColor: accentColor }}
              >
                {selectedCustomers.length}
              </span>
              <span className="text-[13px] text-slate-700">Selected</span>
              <button className="ml-0.5 shrink-0 cursor-pointer rounded-full p-1 text-red-500 transition-colors hover:bg-red-50 hover:text-red-600" onClick={handleClearSelection} aria-label="Clear selection">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 p-3">
          ...
          <div className="flex items-center gap-2">
            <button
              className="inline-flex h-8 w-8 items-center justify-center rounded-lg border-b-[3px] text-white shadow-sm transition-all hover:-translate-y-[1px] hover:brightness-110 active:translate-y-[1px] active:border-b-[1px]"
              style={{ backgroundColor: accentColor, borderBottomColor: accentColor }}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-gradient-to-b from-white to-slate-50/30">
        {sidebarSortedCustomers.map((customerRow: any, index: number) => (
          <div
            key={`${customerRow.id}-${index}`}
            className={`mx-2 my-1 flex cursor-pointer items-center gap-3 border-b border-gray-100 px-3 py-2 transition-all hover:bg-gray-50 ${
              customerRow.id === id ? "rounded-lg bg-blue-50" : "rounded-lg"
            } ${selectedCustomers.includes(customerRow.id) ? "rounded-lg bg-[#f3f5ff]" : ""}`}
            style={
              customerRow.id === id || selectedCustomers.includes(customerRow.id)
                ? { borderLeftWidth: "4px", borderLeftStyle: "solid", borderLeftColor: accentColor }
                : undefined
            }
            onClick={() => {
