...
export default function CustomerDetailSidebar({ detail }: { detail: any }) {
  const {
  ...
  const safeSelectedCustomers = Array.isArray(selectedCustomers) ? selectedCustomers : [];
  const safeCustomers = Array.isArray(customers) ? customers : [];
  const safeSidebarSortedCustomers = Array.isArray(sidebarSortedCustomers) ? sidebarSortedCustomers : [];
  const currentCustomerId = String(customer?._id || customer?.id || routeCustomerId || "").trim();

  return (
    <div className="flex h-full min-h-0 w-[340px] flex-col overflow-visible border-r border-gray-200 bg-white">
      {safeSelectedCustomers.length > 0 ? (
        <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 p-4">
          <div className="flex items-center gap-3">
            <input
              type="checkbox"
              checked={safeSelectedCustomers.length === safeCustomers.length}
            ...
            <div className="relative" ref={bulkActionsDropdownRef}>
              ...
              {isBulkActionsDropdownOpen && (
                <div className="absolute left-0 top-full z-50 mt-1 min-w-[200px] rounded-md border border-gray-200 bg-white shadow-lg">
                  <div className="cursor-pointer px-4 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50" onClick={handleSidebarBulkUpdate}>
                    Bulk Update
                  </div>
                  <div className="cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={handlePrintCustomerStatements}>
                    Print Customer Statements
                  </div>
                  <div className="my-1 h-px bg-gray-200" />
                  <div className="cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={handleSidebarBulkMarkActive}>
                    Mark as Active
                  </div>
                  <div className="cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={handleSidebarBulkMarkInactive}>
                    Mark as Inactive
                  </div>
                  <div className="cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={handleMergeCustomers}>
                    Merge
                  </div>
                  <div className="cursor-pointer px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-50" onClick={handleAssociateTemplates}>
                    Associate Templates
                  </div>
                  <div className="my-1 h-px bg-gray-200" />
                  <div
                  ...
                  <div className="my-1 h-px bg-gray-200" />
                  <div className="cursor-pointer px-4 py-2 text-sm text-red-600 hover:bg-red-50" onClick={handleSidebarBulkDelete}>
                    Delete
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center gap-2">
            <span
              className="flex h-6 min-w-[24px] items-center justify-center rounded px-2 text-xs font-semibold text-white"
              style={{ background: "linear-gradient(90deg, #156372 0%, #0D4A52 100%)" }}
            >
              {safeSelectedCustomers.length}
            </span>
                ...
                <div className="animate-in fade-in slide-in-from-top-1 absolute right-0 top-full z-[110] mt-2 w-60 rounded-lg border border-gray-100 bg-white py-2 shadow-xl duration-200">
                  <div className="group relative flex cursor-pointer items-center gap-3 px-4 py-2.5 text-sm text-gray-700 transition-all hover:bg-[#156372] hover:text-white">
                    ...
                    <div className="pointer-events-none absolute left-full top-0 z-[99999] ml-1.5 w-[220px] translate-x-2.5 rounded-lg border border-gray-200 bg-white py-2 opacity-0 shadow-lg transition-all group-hover:pointer-events-auto group-hover:translate-x-0 group-hover:opacity-100">
                      <div
                        className="flex cursor-pointer items-center justify-between px-4 py-2.5 text-sm text-gray-700 transition-all hover:bg-[#156372] hover:text-white"
                        onClick={() => {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {safeSidebarSortedCustomers.map((customerRow: any, index: number) => (
          <div
            key={`${customerRow.id}-${index}`}
            className={`flex cursor-pointer items-center gap-3 border-b border-gray-100 p-3 hover:bg-gray-50 ${
              customerRow.id === currentCustomerId ? "border-l-4 border-l-blue-600 bg-blue-50" : ""
