...
type GroupByKey = "projects" | "customers" | "tasks" | "staff";

const TIMESHEET_COLUMNS: ColumnGroup[] = [
  {
    label: "Timesheet Columns",
    items: ["Customer", "Task", "Staff", "Notes", "Logged Hours", "Status"],
  },
];
...
const getGroupByLabel = (key: GroupByKey) =>
  GROUP_BY_OPTIONS.find((option) => option.key === key)?.label ?? "Projects";
...
export default function TimesheetDetailsReportPage() {
  ...
  const [selectedColumns, setSelectedColumns] = useState<TimesheetColumn[]>([
    "Customer",
    "Task",
    "Staff",
    "Notes",
  ...
  useEffect(() => {
    const handlePointerDown = (event: MouseEvent) => {
      ...
      if (isGroupByOpen && !groupByMenuRef.current?.contains(target)) setIsGroupByOpen(false);
    };
  ...
  }, [isDateRangeOpen, isExportOpen, isGroupByOpen]);
  ...
  const refreshReport = () => {
    ...
    toast.success("Report refreshed: Timesheet Details");
  };
  ...
  const handleExport = (format: string) => {
    setIsExportOpen(false);
    toast.info(`Exporting Timesheet Details as ${format}`);
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] bg-[#f5f7fc] pt-3">
      <ReportsDrawer
        open={isDrawerOpen}
        activeReportId="timesheet-details"
      ...
      <div className="mx-3 overflow-hidden rounded-[12px] border border-[#e2e8f0] bg-white">
        ...
        <div className="border-b border-[#e6eaf1] px-4 py-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <Columns3 size={14} className="text-[#156372]" />
              More Filters
            </button>
        ...
        {isMoreFiltersOpen ? (
          <div className="border-b border-[#e6eaf1] px-4 py-4">
            <div className="rounded-[12px] border border-[#e6eaf1] bg-white p-4 text-sm text-[#64748b]">
              More filter controls can be added here later. The page already supports the main report actions.
            </div>
          </div>
        ) : null}
          ...
          <div className="overflow-hidden rounded-xl border border-[#d7dce7] bg-white">
            <div className="flex items-center justify-between border-b border-[#e6eaf1] px-4 py-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-[14px] text-[#334155]"
              >
                <Columns3 size={15} className="text-[#64748b]" />
                Group By : <span className="font-medium text-[#0f172a]">{groupByLabel}</span>
                <ChevronDown size={14} />
              </button>
            </div>
            ...
            {isGroupByOpen ? (
              <div
                ref={groupByMenuRef}
                className="absolute right-6 top-[calc(100%+86px)] z-30 w-52 overflow-hidden rounded-md border border-[#d4d9e4] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.14)]"
              >
                {GROUP_BY_OPTIONS.map((option) => {
                  const isSelected = option.key === groupByKey;
                  return (
                    <button
                      key={option.key}
                      type="button"
            ...
            <div className="border-b border-[#eef2f7] px-4 py-10 text-center">
              <div className="text-[13px] text-[#64748b]">{organizationName}</div>
              <h2 className="mt-2 text-[22px] font-semibold text-[#0f172a]">Timesheet Details</h2>
              <div className="mt-1 text-[13px] text-[#475569]">{dateRangeText}</div>
              <div className="mt-3 text-[17px] font-medium text-[#0f172a]">
                {groupByKey === "projects" ? "All Projects" : getGroupByLabel(groupByKey)}
              </div>

              <div className="mx-auto mt-8 max-w-[790px] rounded-[10px] border border-[#e6eaf1] bg-white px-6 py-5 text-left">
                <div className="border-b border-[#eef2f7] pb-4">
                  <div className="text-[12px] font-semibold uppercase tracking-[0.06em] text-[#64748b]">
                    Logged Hours: <span className="text-[#0f172a]">00:00</span>
                  </div>
                  <div className="mt-1 text-[12px] text-[#64748b]">
                    (This includes the time spent on Fixed Rate and Daily Rate projects)
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 pt-4">
                  <div>
                    ...
                    <div className="mt-2 text-[20px] text-[#0f172a]">00:00</div>
                  </div>
                  <div>
                    <div className="text-[12px] font-semibold uppercase tracking-[0.06em] text-[#64748b]">
                      Billed Hours
                    ...
                    {selectedColumns.map((column) => (
                      ...
                      >
                        <span className="inline-flex items-center gap-1">
                          {column}
                          {column === "Customer" ? <ArrowUpDown size={12} /> : null}
                        </span>
                      </th>
                    ))}
                  </tr>
      ...
      <ReportCustomizeColumnsModal
        ...
        reportName="Timesheet Details"
        availableGroups={TIMESHEET_COLUMNS}
        selectedColumns={selectedColumns}
        onClose={() => setIsCustomizeOpen(false)}
        onSave={(nextVisibleColumns) => {
          setSelectedColumns(nextVisibleColumns as TimesheetColumn[]);
