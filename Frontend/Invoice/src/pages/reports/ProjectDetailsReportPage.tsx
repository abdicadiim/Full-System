...
type CustomerKey = "select-customer" | "us-grand-stores" | "jack-wilson" | "ss";
type ReportByKey = "task" | "project" | "customer" | "staff";

const PROJECT_DETAILS_COLUMNS: ColumnGroup[] = [
  {
    label: "Project Details Columns",
    items: [
      "Task Name",
      "Logged Hours",
      "Billable Hours",
...
const CUSTOMER_OPTIONS: Array<{ key: CustomerKey; label: string }> = [
  { key: "select-customer", label: "Select Customer" },
  { key: "us-grand-stores", label: "US Grand Stores" },
  { key: "jack-wilson", label: "Jack Wilson" },
  { key: "ss", label: "ss" },
];

const REPORT_BY_OPTIONS: Array<{ key: ReportByKey; label: string }> = [
  { key: "task", label: "Task" },
...
export default function ProjectDetailsReportPage() {
  ...
  const [selectedColumns, setSelectedColumns] = useState<ProjectDetailsColumn[]>([
    "Task Name",
    "Logged Hours",
    "Billable Hours",
    "Billed Hours",
  ...
  const [customerKey, setCustomerKey] = useState<CustomerKey>("select-customer");
  const [reportByKey, setReportByKey] = useState<ReportByKey>("task");
  ...
  const refreshReport = () => {
    ...
    toast.success("Report refreshed: Project Details");
  };
  ...
  const handleExport = (format: string) => {
    ...
    toast.info(`Exporting Project Details as ${format}`);
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] bg-[#f5f7fc] pt-3">
      <ReportsDrawer
        open={isDrawerOpen}
        activeReportId="project-details"
      ...
      <div className="mx-3 overflow-hidden rounded-[12px] border border-[#e2e8f0] bg-white">
        ...
        <div className="border-b border-[#e6eaf1] px-4 py-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              value={customerKey}
              onChange={(event) => setCustomerKey(event.target.value as CustomerKey)}
              className="h-9 rounded border border-[#cfd6e4] bg-[#f8fafc] px-3 text-[14px] text-[#334155] outline-none focus:border-[#156372]"
            >
            ...
            <select
              value={reportByKey}
              onChange={(event) => setReportByKey(event.target.value as ReportByKey)}
              className="h-9 rounded border border-[#cfd6e4] bg-[#f8fafc] px-3 text-[14px] text-[#334155] outline-none focus:border-[#156372]"
            >
              {REPORT_BY_OPTIONS.map((option) => (
                <option key={option.key} value={option.key}>
                  Report By : {option.label}
                </option>
              ))}
            </select>
            ...
            <button
              type="button"
              onClick={runReport}
              className="inline-flex h-9 items-center gap-1 rounded bg-[#156372] px-4 text-[14px] font-semibold text-white hover:bg-[#0f4f5b]"
            >
              Run Report
            </button>
          </div>
        ...
        <div className="p-3">
          <div className="overflow-hidden rounded-xl border border-[#d7dce7] bg-white">
            ...
            <div className="border-b border-[#eef2f7] px-4 py-10 text-center">
              ...
              <h2 className="mt-2 text-[22px] font-semibold text-[#0f172a]">Project Details</h2>
            </div>
                    ...
                    {selectedColumns.map((column) => (
                      ...
                      >
                        <span className="inline-flex items-center gap-1">
                          {column}
                          {column === "Task Name" ? <ArrowUpDown size={12} /> : null}
                        </span>
                      </th>
                    ))}
                  </tr>
      ...
      <ReportCustomizeColumnsModal
        ...
        reportName="Project Details"
        availableGroups={PROJECT_DETAILS_COLUMNS}
        selectedColumns={selectedColumns}
        onClose={() => setIsCustomizeOpen(false)}
        onSave={(nextVisibleColumns) => {
          setSelectedColumns(nextVisibleColumns as ProjectDetailsColumn[]);
