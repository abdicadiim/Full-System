...
export default function ExpensesByCustomerReportPage() {
  ...
  const rows = useMemo(() => buildRows(expenses, bounds, customerSearch), [bounds.end, bounds.start, customerSearch, expenses]);
  ...
  if (category.id !== "purchases-expenses" || report.id !== "expenses-by-customer") {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("expenses_by_customer_visible_columns_v1", JSON.stringify(selectedColumns));
    } catch {
      // ignore
    }
  }, [selectedColumns]);
