...
const HIDDEN_ITEM_REPORTING_TAG_NAMES = new Set(["cxb", "sdv"]);
  ...
  useEffect(() => {
    ...
    const loadReportingTags = async () => {
      try {
        ...
        const filteredTags = finalTags.filter((tag: any) => {
          const name = String(tag?.name || "").trim().toLowerCase();
          return name && !HIDDEN_ITEM_REPORTING_TAG_NAMES.has(name);
        });
        if (mounted) setAvailableReportingTags(filteredTags);
      } catch (e) {
        if (mounted) setAvailableReportingTags([]);
      }
    };
