...
export const db = {
  init: async () => {
    ...
    seedIfEmpty(tableKeys.users, [
      { id: "usr-1", _id: "usr-1", name: "Taban Admin", email: "admin@taban.local", role: "admin", status: "Active" }
    ] as AnyRecord[]);
    seedIfEmpty(tableKeys.roles, []);
    seedIfEmpty(tableKeys.preferences, []);
    return true;
  },
  ...
  users: new Table(tableKeys.users, "usr"),
  roles: new Table(tableKeys.roles, "role"),
  preferences: new Table(tableKeys.preferences, "pref"),
};
