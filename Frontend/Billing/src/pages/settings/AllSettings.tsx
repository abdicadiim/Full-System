            ...
            {filteredModuleSettings.map((setting, idx) => {
              ...
              return (
                <div
                  ...
                  className={
                    isGeneralCard
                      ? "bg-white rounded-xl border border-gray-200 hover:shadow-md transition-shadow overflow-hidden"
                      : "bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow"
                  }
                >
                  <div className={isGeneralCard ? "flex items-center gap-3 px-4 py-3 bg-green-50" : "flex items-center gap-3 mb-3"}>
                    <div
                      className={`w-10 h-10 rounded-lg ${getColorClasses(setting.color)} flex items-center justify-center`}
                    >
                      <Icon size={20} />
                    </div>
                    <div>
                      <h3 className="font-semibold text-gray-900">{setting.title}</h3>
                      {!isGeneralCard && setting.description && (
                        <p className="text-xs text-gray-500 mt-0.5 max-w-[260px]">{setting.description}</p>
                      )}
                    </div>
                  </div>
                  <ul className={isGeneralCard ? "px-4 py-3 space-y-2" : "space-y-1"}>
                    {setting.items.map((item, itemIdx) => (
                      <li
                        key={itemIdx}
                        onClick={() => {
                        ...
                        className={
                          isGeneralCard
                            ? "text-sm text-gray-700 hover:text-[#156372] cursor-pointer py-1"
                            : "text-sm text-gray-600 hover:text-blue-600 cursor-pointer py-1"
                        }
                      >
                        {item}
                      </li>
