...
const SAMPLE_HEADERS = ["Product Name", "Description", "Status", "Redirect URL", "Notification Mail Address"];
const SAMPLE_ROWS = [
  ["Cloud Box", "A cloud-based mail storage service", "Active", "https://www.example.com", "test@zylker.org"],
  ["Sample Product", "", "Inactive", "", "test@zylker.org,johndoe@zylker.org"],
];

const IMPORT_FIELDS: FieldDef[] = [
  { key: "name", label: "Product Name", required: true, aliases: ["product name", "name", "product"] },
  { key: "description", label: "Description", aliases: ["description", "desc"] },
  { key: "status", label: "Status", aliases: ["status"] },
  { key: "redirectUrl", label: "Redirect URL", aliases: ["redirect url", "redirection url", "redirect"] },
  { key: "notificationMailAddress", label: "Notification Mail Address", aliases: ["notification mail address", "notification email", "email recipients"] },
];
...
function StepBadge({ step, current, label }: { step: Step; current: Step; label: string }) {
  ...
  return (
    <div className={`flex items-center gap-2 ${!active && !done ? "text-[#c0c7d8]" : "text-[#111827]"}`}>
      <span
        className={`flex h-7 w-7 items-center justify-center rounded-full text-xs ${done ? "bg-[#16a34a] text-white" : active ? "bg-[#3b82f6] text-white" : "border border-[#d7dce8]"
          }`}
      >
...
export default function ImportProductsPage() {
  ...
  const handleDownloadSample = () => {
    const csvContent = [
      SAMPLE_HEADERS.map(h => `"${h.replace(/"/g, '""')}"`).join(","),
      ...SAMPLE_ROWS.map(row => row.map(cell => `"${cell.replace(/"/g, '""')}"`).join(","))
    ].join("\n");
  ...
  const stepTitle = step === 1 ? "Products - Select File" : step === 2 ? "Map Fields" : "Preview";
    ...
    <div className="min-h-[calc(100vh-96px)] bg-[#f6f7fb] p-4 md:p-6">
      <div className="rounded-lg border border-[#d8deea] bg-white">
        <div className="relative border-b border-[#e3e7f2] px-4 py-4 md:px-6">
          ...
          <button type="button" onClick={() => navigate("/products/plans?tab=products")} className="absolute right-4 top-4 text-[#ef4444] hover:text-[#dc2626]">
            <X size={18} />
          </button>
        </div>
          ...
          <div className="px-4 py-8 md:px-6">
            <div className="mx-auto w-full max-w-[1060px]">
              <div className="mb-6 rounded-lg bg-[#e8effa] px-4 py-3 text-sm text-[#334155]">
                All Products in your file are ready to be imported
              </div>
              ...
              <div className="divide-y divide-[#e3e7f2] border-y border-[#e3e7f2]">
                <div className="flex items-center justify-between py-3 text-[18px] text-[#334155]">
                  <div>Products that are ready to be imported - {preview.ready.length}</div>
                    ...
                    <table className="w-full border-collapse text-left">
                      <thead>
                        <tr className="bg-[#f8fafc]">
                          <th className="border border-[#e5e7eb] px-3 py-2 text-xs font-semibold text-[#64748b]">Product Name</th>
                          <th className="border border-[#e5e7eb] px-3 py-2 text-xs font-semibold text-[#64748b]">Description</th>
                          <th className="border border-[#e5e7eb] px-3 py-2 text-xs font-semibold text-[#64748b]">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {preview.ready.slice(0, 10).map((row, idx) => (
                          <tr key={`${row.name}-${idx}`}>
                            <td className="border border-[#e5e7eb] px-3 py-2 text-sm">{row.name}</td>
                            <td className="border border-[#e5e7eb] px-3 py-2 text-sm">{row.description || "-"}</td>
                            <td className="border border-[#e5e7eb] px-3 py-2 text-sm">{row.status || "Active"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <div className="flex items-center justify-between border-t border-[#e3e7f2] bg-white px-4 py-3 md:px-6">
          ...
          <button
            type="button"
            onClick={() => navigate("/products/plans?tab=products")}
