...
export default function TimeToGetPaidReportPage() {
  ...
  const reportColumnGroups = useMemo(() => getReportColumnGroups("time-to-get-paid", resolvedCategoryId), [resolvedCategoryId]);
  const filtersRef = useRef<HTMLDivElement>(null);

  const [selectedDateRange, setSelectedDateRange] = useState<DateRangePreset>("This Month");
  const [customRange, setCustomRange] = useState<RangeDate>(() => getDefaultCustomRange());
  const [isDateRangeOpen, setIsDateRangeOpen] = useState(false);
  const [visibleColumns, setVisibleColumns] = useState<string[]>(REPORT_COLUMNS);
  ...
  const selectedRangeBounds = useMemo(
    () => getDateRangeBounds(selectedDateRange, customRange),
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [payments, invoices] = await Promise.all([
        fetchCollectionRows("/payments-received"),
  ...
  }, [refreshTick, selectedRangeBounds.end.getTime(), selectedRangeBounds.start.getTime()]);
  ...
  const visiblePreviewColumns = visibleColumns.length > 0 ? visibleColumns : previewColumns;
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col gap-3 overflow-hidden">
      <section className="rounded-[16px] border border-[#d7dce7] bg-white shadow-sm">
        ...
        <div ref={filtersRef} className="relative flex flex-wrap items-center gap-2 border-t border-[#e6e9f0] px-4 py-2">
